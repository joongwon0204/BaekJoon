...
int bfs(int n, int m, int k, vector<vector<int>>& base, vector<vector<vector<int>>>& visited) {
    ...
    while (!q.empty()) {
        ...
        for (auto& move: moves) {
            ...
            } else if (base[new_y][new_x] == 1 && broken < k && visited[new_y][new_x][broken+1] == 0) {
                q.push({new_y, new_x, broken+1});
                visited[new_y][new_x][broken+1] = visited[y][x][broken] + 1;
            }
        }
    }
